...
# Target encode Type columns
def target_encode(df, column, target_column):
    # Calculate the mean target value for each category
    means = df.groupby(column)[target_column].mean()
    # Replace the category with its corresponding mean
    return df[column].map(means)


# Apply target encoding to Type columns
